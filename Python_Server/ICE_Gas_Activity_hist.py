...
path = 'https://downloads.theice.com/EOD_Gas_CSV/gasactivity_'
    ...
    for m in range(len(mn)):
        ...
        for d in range(len(dn)):
            try:
                ...
                df.to_sql('ICEGASact_Landing', engine, schema = 'dbo', index = False, if_exists='replace')
                time.sleep(3)
                sql = 'EXEC dbo.EODGas_Upload'
