...
while x<3:
    try:
        response = requests.get(url5, timeout=10)
        thezip = ZipFile(io.BytesIO(response.content))
        a = thezip.namelist()
...
while x<3:
    try:
        ...
        response = requests.get(url6, timeout=10)
        thezip = ZipFile(io.BytesIO(response.content))
        a = thezip.namelist()
...
dt.to_sql('dbo.CAISO_DA_VR_Landing', engine, schema = 'dbo', index = False, if_exists='replace')
